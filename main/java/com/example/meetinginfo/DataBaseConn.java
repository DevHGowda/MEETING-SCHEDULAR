...
public class DataBaseConn extends SQLiteOpenHelper {
    ...
    public String fetchi(String d){

        SQLiteDatabase DB=this.getReadableDatabase();
        // String sqlquery="select name from MDTbl where date='19/3/21' AND time='Afternoon'";
        // Cursor c = DB.rawQuery(sqlquery,null);
        String t = String.valueOf(DB.rawQuery("Select time from meetingTbl where date='"+d+"' ",null));
        return t;
    }

    public boolean deleteValues(String d) {
        SQLiteDatabase DB = this.getWritableDatabase();
        int rowsDeleted = DB.delete("meetingTbl", "date = ?", new String[]{d});
